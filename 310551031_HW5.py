...
def testing(test_loader, model, device):
    ...
    global y_pred_All_test_batch
    global y_true_All_test_batch
    test_accuracy = []
    y_pred = []
    model.eval()
    with torch.no_grad():
        ...
        for i, (x_test, y_test) in curloader:
            ...
            correct_test = (torch.max(y_pred_test, 1)[
                            1] == y_test).sum().item()
            y_pred_All_test_batch += (torch.max(y_pred_test, 1)
                                      [1].cpu().numpy().tolist())
            y_pred += (torch.max(y_pred_test, 1)
                       [1].cpu().numpy().tolist())

            test_accuracy.append(correct_test/n)
            curloader.set_postfix(accuracy=(correct_test/n))
...
if opt.mode == "train":
    ...
    for epoch in range(epochs):
        ...
        test_accuracy = testing(test_loader, model, device)
        ...
        print("\n epochs:", epoch, "Training loss:", train_loss, "Training Accuracy:",
              train_accuracy, "Testing Accuracy:", test_accuracy)
    ...
    df = pd.DataFrame({"loss": loss_history, "train_accuracy_history": train_accuracy_history,
